...
def handle_generate(message):
    ...
    try:
        ...
        if not input_file.exists():
            bot.edit_message_text("❌ Помилка: Файл не згенерувався.", chat_id=message.chat.id,
        ...
        bot.edit_message_text("🧠 Діалоги згенеровано! Тепер аналізую їх через Gemini...", chat_id=message.chat.id,
                              message_id=status_msg.message_id)

        process_dataset(message.chat.id, dataset, status_msg.message_id)

    except Exception as e:
        bot.edit_message_text(f"❌ Помилка під час генерації: {e}", chat_id=message.chat.id,
                              message_id=status_msg.message_id)
